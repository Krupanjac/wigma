...
-- Projects: members can read, owner/editors can write
...
CREATE POLICY projects_update ON projects FOR UPDATE USING (
  EXISTS (SELECT 1 FROM project_users pu WHERE pu.project_id = id AND pu.user_id = auth.uid() AND pu.role IN ('owner', 'editor'))
);
...
CREATE POLICY project_users_select ON project_users FOR SELECT USING (
  EXISTS (SELECT 1 FROM project_users pu2 WHERE pu2.project_id = project_id AND pu2.user_id = auth.uid())
);
...
CREATE POLICY yjs_snapshots_select ON yjs_snapshots FOR SELECT USING (
  EXISTS (SELECT 1 FROM project_users pu WHERE pu.project_id = yjs_snapshots.project_id AND pu.user_id = auth.uid())
);
CREATE POLICY yjs_snapshots_upsert ON yjs_snapshots FOR ALL USING (
  EXISTS (SELECT 1 FROM project_users pu WHERE pu.project_id = yjs_snapshots.project_id AND pu.user_id = auth.uid() AND pu.role IN ('owner', 'editor'))
);

CREATE POLICY yjs_updates_select ON yjs_updates FOR SELECT USING (
  EXISTS (SELECT 1 FROM project_users pu WHERE pu.project_id = yjs_updates.project_id AND pu.user_id = auth.uid())
);
CREATE POLICY yjs_updates_insert ON yjs_updates FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM project_users pu WHERE pu.project_id = yjs_updates.project_id AND pu.user_id = auth.uid() AND pu.role IN ('owner', 'editor'))
);

-- Media: project members can read, editors can upload
CREATE POLICY media_select ON media_files FOR SELECT USING (
  EXISTS (SELECT 1 FROM project_users pu WHERE pu.project_id = media_files.project_id AND pu.user_id = auth.uid())
);
CREATE POLICY media_insert ON media_files FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM project_users pu WHERE pu.project_id = media_files.project_id AND pu.user_id = auth.uid() AND pu.role IN ('owner', 'editor'))
);
